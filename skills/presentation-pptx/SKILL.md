...
description: "Genere des presentations PowerPoint (.pptx) de niveau professionnel avec python-pptx dans le sandbox d'execution de code : slides titre, section, puces, deux colonnes, image, tableau et graphique natif, avec une charte graphique imposee. Utiliser cette skill des que l'utilisateur demande une presentation, un deck, des slides, un pitch, un support de reunion ou un fichier .pptx, ou des qu'il faut produire ou modifier un fichier .pptx. Ne pas utiliser pour : des slides web ou une presentation a afficher dans le navigateur (renvoyer vers la skill html-slides), un fichier Excel ou un tableur (renvoyer vers tableur-excel), un document Word ou un rapport texte (renvoyer vers document-word)."
when-to-use: "Quand le livrable attendu est un fichier PowerPoint .pptx : creation d'un deck complet a partir d'un brief, mise en forme soignee de slides, ajout de graphiques natifs, tableaux, images et notes d'orateur, puis conversion optionnelle en PDF. Pas pour du HTML, de l'Excel ou du Word."
always-apply: false
---
